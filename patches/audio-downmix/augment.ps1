...
foreach ($requiredPath in @($javac, $jar, $buildTools, $androidJar, $sourceDirectory)) {
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "Required audio-downmix module path does not exist: $requiredPath"
    }
}

if ($sources.Count -eq 0) {
    throw "The audio-downmix module contains no Java sources under $sourceDirectory"
}

$classes = Join-Path $BuildDirectory 'classes'
$dex = Join-Path $BuildDirectory 'dex'
$moduleJar = Join-Path $BuildDirectory 'audio-downmix.jar'
...
if ($LASTEXITCODE -ne 0) {
    throw 'javac failed to compile the audio-downmix module.'
}

& $jar cf $moduleJar -C $classes '.'
if ($LASTEXITCODE -ne 0) {
    throw 'Failed to package the audio-downmix module classes.'
}
...
& (Join-Path $buildTools 'd8.bat') `
    ...
    $moduleJar
if ($LASTEXITCODE -ne 0) {
    throw 'D8 failed to build the audio-downmix module dex.'
}

Move-Item -LiteralPath (Join-Path $dex 'classes.dex') -Destination (Join-Path $dex 'classes12.dex')

Push-Location $dex
try {
    & $jar uf $artifact 'classes12.dex'
    if ($LASTEXITCODE -ne 0) {
        throw 'Failed to inject the audio-downmix module dex into the APK.'
    }
}
finally {
    Pop-Location
